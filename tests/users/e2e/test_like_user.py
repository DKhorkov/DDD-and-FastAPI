...
async def test_like_user_success(
    ...
    like_user_url_prefix: str = get_substring_before_chars(
        chars='{',
        string=URLPathsConfig.LIKE_USER
    )

    like_user_url_postfix: str = get_substring_after_chars(
        chars='}',
        string=URLPathsConfig.LIKE_USER
    )

    response: Response = await async_client.patch(
        url=RouterConfig.PREFIX + like_user_url_prefix + '2' + like_user_url_postfix,
        cookies=cookies
    )
    ...
    assert response_content['likes'] == 1
    assert response_content['dislikes'] == 0


@pytest.mark.anyio
async def test_like_user_fail_user_not_authorized(async_client: AsyncClient, create_test_user: None) -> None:
...
@pytest.mark.anyio
async def test_like_user_fail_voted_for_user_does_not_exists(
        async_client: AsyncClient,
        create_test_user: None,
        cookies: Cookies
) -> None:
...
@pytest.mark.anyio
async def test_like_user_fail_user_can_not_vote_for_himself(
        async_client: AsyncClient,
        create_test_user: None,
        cookies: Cookies
) -> None:
    ...
    response: Response = await async_client.patch(
        url=RouterConfig.PREFIX + like_user_url_prefix + '1' + like_user_url_postfix,
        cookies=cookies
    )
